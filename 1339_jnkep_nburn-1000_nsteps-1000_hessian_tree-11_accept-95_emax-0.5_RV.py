...
period_guess = [8.88, 28.579657965796578, 38.35013501350135]
for j in range(3):
...
print(popt)
print(param_bounds)
with open('1339_jnkep_initial_fit_data_hessian_tree-11_accept-95_emax-0.5_RV.pkl', 'wb') as f:
    pickle.dump({'jttv': jttv, 'popt': popt, 'param_bounds': param_bounds}, f)

print('Initial fit data saved successfully')
##################################################################################################
TESS_offset = 2457000
rv_data_file = "ajad79edt3_mrt.txt"
...
def model_scaled(sample_keys, param_bounds):
    ...
    times_rv = jnp.array(df_rv["BJD"].values)  # times in TESS offset days
    tcmodel, rvmodel, ediff = jttv.get_transit_times_and_rvs_obs(par, times_rv)
    numpyro.deterministic("ediff", ediff)
    numpyro.deterministic("tcmodel", tcmodel)
    ...
    # RV likelihood
    rv_obs = jnp.array(df_rv["RVel"].values)
    rv_err = jnp.array(df_rv["e_RVel"].values)
...
import dill
with open("1339_jnkep_nburn-1000_nsteps-1000_hessian_tree-11_accept-95_emax-0.5_RV.pkl", "wb") as f:
    dill.dump(mcmc, f)
